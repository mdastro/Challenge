...
"""
    ...
    Corrections and forced 5 decimals to the wl column.
    ...
    for each_model in emiles_models_list:
        ...
        for w in range(wavelength.size):
            if w == 0:
                ...
                new_wavelength_i = str(new_wavelength_i)+'0000'
            elif w > 0:
                ...
                if new_wavelength_temp == round(previous_wavelength+0.9, 1):
                    new_wavelength_i = str(new_wavelength_temp)+'0000'
                else:
        ...
        # new_model      = np.ma.row_stack(((header, new_model)))
        new_model_df   = pd.DataFrame(new_model)
        # new_model_df['wavelength'] = new_model_df['wavelength'].map(lambda x: '{0:.5}'.format(x))
        new_model_df.to_csv(os.path.join(emiles_models_path, os.path.splitext(each_model)[0:4][0]+'.csv'), sep=',',
                             header= header,index=False, float_format='%.5f')
